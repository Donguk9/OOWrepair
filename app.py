...
elif 7 <= current_month <= 9: target_q = "Q3"
else: target_q = "Q4"
rate_df['Exchange'] = rate_df[target_q]
print(f"현재는 {current_month}월이므로 {target_q} 데이터를 'Exchange'필드로 가져왔습니다.")
    ...
    if match:
        ...
        try:
            ...
            for item in data_list:
                print(f"모델명: {item['modelCode']}, 가격: {item.get('price', '정보없음')}")
...
def samcol():
    ...
    if match:
        ...
        try:
            data_list = ast.literal_eval(f"[{items_str}]")
            for item in data_list:
                print(f"모델명: {item['display_name']}, 가격: {item.get('price', '정보없음')}")
...
def model_clean(name):
    ...
    if re.search(r'\d+L|\d+”|UHD|QLED|OLED|Smart TV|Bespoke', name, re.I):
        return None

    # 2. 괄호 안의 정보 확인 (Inner, Outer 추출용)
    display_info = ""
    if "Inner" in name or "inner" in name: display_info = " (Inner)"
    elif "Outer" in name or "outer" in name: display_info = " (Outer)"

    # 3. SM-코드 (이름) 형태 처리: 괄호 안에 모델명이 있으면 그걸 우선 사용
    bracket_match = re.search(r'\(([^)]+)\)', name)
    if bracket_match: 
        content = bracket_match.group(1)
        if not re.search(r'Inner|Outer', content, re.I): 
            name = content 

    # 4. 불필요한 접두사 제거 (Galaxy, Galaxy-, G- 등)
    name = re.sub(r'Galaxy[- ]?|G-|SM-[A-Z0-9]+', '', name).strip()

    # 5. Z시리즈 보정 (Flip/Fold만 있으면 Z 붙이기)
    name = re.sub(r'^(Flip|Fold)', r'Z \1', name, flags = re.I)

    # 6. A,S,Z시리즈 핵심 패턴 추출
    pattern = r'^([AS]\d+|Z\s?(Flip|Fold)\s?\d?)(.*)'
