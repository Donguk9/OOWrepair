오케이. 지금까지 미국 사이트 대상으로 완벽하게 됐어. 이제 다른 법인들을 추가해야 함.

여기서 고민 : 50개 국가가 있는데, 사이트마다 ux가 다름. 그래서 같은 표 형식일지라도 내용이 다르거나, 아니면 아예 드롭다운 형식으로 되어있는 곳도 있음. 이미지도 있음 심지어. 그렇다면 나는 코드를 국가별로 작성해야 하는가? 아니면 통합 코드를 작성해야 하는가? 근데 통합 코드가 된다고 해도 너무 복잡할까봐 고민. 아님 함수를 기능별로 나눠서 모듈화? 뭐가 가장 효율적일까
